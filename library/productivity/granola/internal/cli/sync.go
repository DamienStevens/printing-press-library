...
Resource scoping:
  ...
  granola-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  granola-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  granola-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  granola-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  granola-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if since != "" {
				...
				// Granola's REST date filters (created_after / updated_after)
				// reject a numeric zone offset (…-04:00 → 400 "Invalid date");
				// force UTC (…Z). Printed-CLI re-application of the generator
				// fix tracked upstream in cli-printing-press#3672.
...
func responsePathForResource(resource, path string) []string {
	...
	case "folders\x00/v1/folders":
		return []string{"folders"}
	case "notes\x00/v1/notes":
		return []string{"notes"}
	}
	return nil
}
