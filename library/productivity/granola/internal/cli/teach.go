...
// granola-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/productivity/granola/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/productivity/granola/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/productivity/granola/internal/learn/patterns"
...
const noLearnEnvVar = "GRANOLA_NO_LEARN"
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  granola-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  granola-pp-cli recall "<question>" --agent
  granola-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  granola-pp-cli learnings list --agent
  granola-pp-cli learnings list --query "<substring>"
  granola-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  granola-pp-cli learnings forget "<question>" --resource <id>
  granola-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  granola-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  granola-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
