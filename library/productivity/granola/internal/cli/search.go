...
Run sync first to populate the local search index.`,
  ...
  granola-pp-cli search "status"

  # Force local search explicitly
  granola-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  granola-pp-cli search "status" --type folders --data-source local
  # JSON output for piping
  granola-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'granola-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
