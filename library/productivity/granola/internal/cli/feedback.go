...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GRANOLA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GRANOLA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "granola-pp-cli/feedback")
...
When ` + "`GRANOLA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GRANOLA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  granola-pp-cli feedback list
  granola-pp-cli feedback list --limit 5
  granola-pp-cli feedback list --json`,
