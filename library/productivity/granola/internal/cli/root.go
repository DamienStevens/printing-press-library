...
type rootFlags struct {
	...
	dryRun  bool
	noCache bool
	// noRefresh skips the granola auto-refresh (REST store hydration) that
	// PersistentPreRunE fires before every command. Mirrors GRANOLA_NO_AUTO_REFRESH.
	noRefresh bool
	noInput   bool
	yes       bool
	agent     bool
	// noLearn disables both teach (write) and recall (read) for this
	// invocation. Mirrors the GRANOLA_NO_LEARN env var.
	noLearn           bool
	selectFields      string
	configPath        string
	homePath          string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "granola-pp-cli",
		Short: `Granola CLI — Every Granola feature — plus offline SQLite cross-meeting search, attendee timelines, and a MEMO pipeline runner no…`,
		Long: `Granola CLI — Every Granola feature — plus offline SQLite cross-meeting search, attendee timelines, and a MEMO pipeline runner no other Granola tool has.

Add --agent to any command for JSON output + non-interactive mode.
Run 'granola-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'granola-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		// Granola auto-refresh: best-effort REST store hydration before every
		// command. Self-gates via autoRefreshOptedOut + noRefreshCommands;
