...
type Config struct {
	...
	GranolaApiKey    string          `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("GRANOLA_API_KEY"); v != "" {
		cfg.GranolaApiKey = v
		cfg.markEnvOverride("GranolaApiKey")
		cfg.AuthSource = "env:GRANOLA_API_KEY"
		cfg.CredentialSource = "env:GRANOLA_API_KEY"
	}
	...
	if v := os.Getenv("GRANOLA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("GRANOLA_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "granola-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.GranolaApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GRANOLA_API_KEY"
		}
		return "Bearer " + c.GranolaApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.GranolaApiKey = creds.GranolaApiKey
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.GranolaApiKey = ""
	delete(c.envOverrides, "GranolaApiKey")
	c.updateFileConfigField("GranolaApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GranolaApiKey"] {
			out.GranolaApiKey = c.fileConfig.GranolaApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GranolaApiKey":
		c.fileConfig.GranolaApiKey = c.GranolaApiKey
	}
}
