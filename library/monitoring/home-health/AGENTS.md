...
This directory is a generated `home-health-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
home-health-pp-cli doctor --json
home-health-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
home-health-pp-cli which "<capability>" --json
home-health-pp-cli <command> --help
```
...
```bash
home-health-pp-cli <command> --agent
```
...
```bash
home-health-pp-cli <command> --help
home-health-pp-cli <command> --dry-run --agent
```
