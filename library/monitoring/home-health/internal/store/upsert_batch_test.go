...
func TestUpsertBatch_PopulatesSamplesTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "nodes_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "nodes_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "nodes_id": "test-parent-001"}`),
	}
...
// TestUpsertBatch_TypedFailureDoesNotStrandSamplesGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandSamplesGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "nodes_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("samples", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("samples count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "nodes_id")
	}
}

// TestUpsertBatch_SetsSamplesParentID verifies that dependent-resource
...
func TestUpsertBatch_SetsSamplesParentID(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	var matchedA int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE parent_id = ?`, "samples")
