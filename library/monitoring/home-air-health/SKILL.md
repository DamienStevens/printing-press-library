...
This skill drives the `home-air-health-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer:
   ```bash
   npx -y @mvanhorn/printing-press-library install home-health --cli-only
   ```
2. Verify: `home-air-health-pp-cli --version`
...
- `home-air-health-pp-cli collect` — Pull the latest readings from every configured source into the local SQLite history. `--since 168h` backfills MOCREO. Idempotent.
- `home-air-health-pp-cli dashboard` — Whole-home view over a period. `--period day|week|month|quarter|year|all`, `--focus mold|allergy|all`, with `--room`, `--metric`, `--source` filters.
- `home-air-health-pp-cli rooms` — List every monitored room with its latest readings.
- `home-air-health-pp-cli sensor <room>` — Detailed readings for one room over a window (`--period`).
- `home-air-health-pp-cli doctor` — Verify config, cache, and per-source credential status.
- `home-air-health-pp-cli auth` — Manage the MOCREO login token (other sources read from the Keychain).
- `home-air-health-pp-cli search` / `export` / `import` — Query, back up, or load the local history.
...
```bash
home-air-health-pp-cli dashboard --period month --focus mold --agent --select flags,mold
```
...
```bash
home-air-health-pp-cli sensor Kitchen --period year
```
...
Run `home-air-health-pp-cli doctor` to confirm which sources resolved. See the README for the full setup and the launchd auto-collect schedule.
...
```
home-air-health-pp-cli feedback "dashboard --focus allergy omitted PM10 for an outdoor sensor"
home-air-health-pp-cli feedback list --json --limit 10
```

Entries are stored locally at `~/.local/share/home-air-health-pp-cli/feedback.jsonl` and never sent unless explicitly opted in. Write what *surprised* you, one line — that is the part that compounds.
...
1. **Empty, `help`, or `--help`** → show `home-air-health-pp-cli --help` output
...
```bash
claude mcp add home-air-health-pp-mcp -- home-air-health-pp-mcp
```
...
1. Check if installed: `which home-air-health-pp-cli`. If not found, offer to install (see Prerequisites).
2. Match the user query to the best command from the Command Reference and Recipes above.
3. Execute with the `--agent` flag:
   ```bash
   home-air-health-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `home-air-health-pp-cli <command> --help`.
