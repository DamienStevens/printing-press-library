...
func newDashboardCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  home-air-health-pp-cli dashboard --period week --focus mold\n" +
			"  home-air-health-pp-cli dashboard --period quarter --focus allergy\n" +
			"  home-air-health-pp-cli dashboard --period day --room Bedroom,Crawlspace\n" +
			"  home-air-health-pp-cli dashboard --period month --json --agent",
...
func renderDashboard(cmd *cobra.Command, v dashboardJSON) {
	...
	period := v.Period
	if len(period) > 0 {
		period = strings.ToUpper(period[:1]) + period[1:]
	}
	fmt.Fprintf(out, "Home Health — %s (%s), focus: %s\n", period, span, v.Focus)
	fmt.Fprintf(out, "%d readings across %d rooms\n", v.Readings, len(v.Rooms))

	if v.Readings == 0 {
		// An empty store must not render as "all clear" — that hides the
		// first-run state where collect has never been executed.
		fmt.Fprintln(out, "\nNo readings yet — run `home-air-health-pp-cli collect` first.")
		return
	}
