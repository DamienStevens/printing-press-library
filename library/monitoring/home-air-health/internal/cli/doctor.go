...
// doctorSources lists the optional per-source credentials the doctor reports,
// paired with a human label. Each source is optional; the product works with
// any subset configured.
var doctorSources = []struct {
	service string
	label   string
}{
	{cred.ServiceAirThings, "AirThings"},
	{cred.ServiceIQAir, "IQAir/AirVisual"},
	{cred.ServiceMOCREO, "MOCREO"},
}

// collectSourceReport probes each optional source's Keychain credential and
// returns a per-source status map plus a count of how many are configured.
// A read error is reported per-source without failing the whole doctor run.
func collectSourceReport(ctx context.Context) (map[string]any, int) {
	out := map[string]any{}
	configured := 0
	for _, s := range doctorSources {
		c, found, err := cred.Get(ctx, s.service)
		switch {
		case err != nil:
			out[s.label] = fmt.Sprintf("error reading credential: %s", err)
		case found:
			configured++
			out[s.label] = fmt.Sprintf("configured (account=%s)", c.Account)
		default:
			out[s.label] = "not configured (optional)"
		}
	}
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  home-air-health-pp-cli doctor
  home-air-health-pp-cli doctor --json
  home-air-health-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export HOME_HEALTH_SYNCSIGN_AUTH2=<your-key>"
					report["auth_key_url"] = "https://portal.mocreo.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("HOME_HEALTH_SYNCSIGN_AUTH2") != "" {
				authEnvSet = append(authEnvSet, "HOME_HEALTH_SYNCSIGN_AUTH2")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "HOME_HEALTH_SYNCSIGN_AUTH2")
			}
			switch {
			...
			// Per-source credential status. All three sources are OPTIONAL —
			// none configured is a warning (nothing to collect), at least one
			// configured is healthy. Reported as a map for the JSON envelope
			// and rendered as its own section in the human output.
			sourceReport, sourcesConfigured := collectSourceReport(cmd.Context())
			report["sources"] = sourceReport
			if sourcesConfigured == 0 {
				report["sources_summary"] = "WARN no sources configured (optional) — add credentials for any sensor you own; see README"
			} else {
				report["sources_summary"] = fmt.Sprintf("OK %d of %d sources configured", sourcesConfigured, len(doctorSources))
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "home-air-health-pp-cli"
						verifyPath := "/user"
			...
			// Per-source credential section. Render the summary line (OK/WARN)
			// then each source's status indented beneath it.
			if summary, ok := report["sources_summary"].(string); ok {
				indicator := green("OK")
				if strings.HasPrefix(summary, "WARN") {
					indicator = yellow("WARN")
				}
				fmt.Fprintf(w, "  %s Sources: %s\n", indicator, summary)
				for _, src := range doctorSources {
					if v, ok := sourceReport[src.label].(string); ok {
						fmt.Fprintf(w, "    - %s: %s\n", src.label, v)
					}
				}
			}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("home-air-health-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'home-air-health-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'home-air-health-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'home-air-health-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'home-air-health-pp-cli sync' to refresh."
	}
	return report
}
