...
func newRoomsCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example:     "  home-air-health-pp-cli rooms\n  home-air-health-pp-cli rooms --json",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			out := cmd.OutOrStdout()
			if len(summaries) == 0 {
				fmt.Fprintln(out, "No readings yet — run `home-air-health-pp-cli collect` first.")
				return nil
			}
...
func newSensorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     "  home-air-health-pp-cli sensor Crawlspace --period month\n  home-air-health-pp-cli sensor Bedroom --json",
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			...
			if len(agg) == 0 {
				return fmt.Errorf("no readings for room %q in this window — run `home-air-health-pp-cli collect`, or see `rooms` for known rooms", room)
			}
			if flags.asJSON || flags.agent {
