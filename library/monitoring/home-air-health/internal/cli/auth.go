...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage the MOCREO login token (other sources use the macOS Keychain)",
		Long: `Manage the MOCREO login token.

Credentials for all three sources (AirThings, IQAir/AirVisual, MOCREO) are
stored in the macOS Keychain — see the README for the 'security
add-generic-password' commands. This 'auth' subcommand only handles the
MOCREO login token written to the config file; AirThings and IQAir are read
directly from the Keychain by 'collect'.`,
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  home-air-health-pp-cli auth setup\n  home-air-health-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "MOCREO login token. Get it from the MOCREO portal: https://portal.mocreo.com")
			fmt.Fprintln(w, "(AirThings and IQAir credentials go in the macOS Keychain — see the README.)")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export HOME_HEALTH_SYNCSIGN_AUTH2=\"<your-token>\"")
			fmt.Fprintln(w, "  home-air-health-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://portal.mocreo.com"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  home-air-health-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export HOME_HEALTH_SYNCSIGN_AUTH2=\"your-token-here\"")
				fmt.Fprintf(w, "  home-air-health-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  home-air-health-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  home-air-health-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("HOME_HEALTH_SYNCSIGN_AUTH2") != "" {
				envStillSet = "HOME_HEALTH_SYNCSIGN_AUTH2"
			}
