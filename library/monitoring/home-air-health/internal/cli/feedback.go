...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "home-air-health-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HOME_HEALTH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HOME_HEALTH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "home-air-health-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/home-air-health-pp-cli/feedback.jsonl.
When ` + "`HOME_HEALTH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HOME_HEALTH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  home-air-health-pp-cli feedback list
  home-air-health-pp-cli feedback list --limit 5
  home-air-health-pp-cli feedback list --json`,
