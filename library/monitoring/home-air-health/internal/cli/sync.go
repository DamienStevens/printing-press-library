...
Resource scoping:
  ...
  home-air-health-pp-cli sync

  # Sync specific resources only
  home-air-health-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  home-air-health-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  home-air-health-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  home-air-health-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  home-air-health-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/home-air-health-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "nodes":
		return db.UpsertNodes(data)
	case "samples":
		return db.UpsertSamples(data)
	case "oauth":
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "samples", ParentTable: "nodes", ParentIDParam: "node_id", PathTemplate: "/nodes/{node_id}/samples", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "node_id", Field: "id"},
		}},
	}
}
