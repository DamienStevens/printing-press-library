...
type Config struct {
	...
	Path                    string            `toml:"-"`
	HomeHealthSyncsignAuth2 string            `toml:"health_syncsign_auth2"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("HOME_HEALTH_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "home-air-health-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("HOME_HEALTH_SYNCSIGN_AUTH2"); v != "" {
		cfg.HomeHealthSyncsignAuth2 = v
		cfg.AuthSource = "env:HOME_HEALTH_SYNCSIGN_AUTH2"
	}
	...
	if cfg.AuthSource == "" && cfg.HomeHealthSyncsignAuth2 != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("HOME_HEALTH_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.HomeHealthSyncsignAuth2
	if token == "" {
		return ""
	}
	if c.HomeHealthSyncsignAuth2 == "" {
		return ""
	}
	return token
}
...
func (c *Config) SaveCredential(token string) error {
	c.AuthHeaderVal = ""
	c.AccessToken = ""
	c.HomeHealthSyncsignAuth2 = token
	return c.save()
}
