...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "aws-billing-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AWS_BILLING_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AWS_BILLING_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "aws-billing-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/aws-billing-pp-cli/feedback.jsonl.
When ` + "`AWS_BILLING_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AWS_BILLING_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  aws-billing-pp-cli feedback list
  aws-billing-pp-cli feedback list --limit 5
  aws-billing-pp-cli feedback list --json`,
