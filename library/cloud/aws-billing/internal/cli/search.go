...
In local mode: searches locally synced data only.`,
  ...
  aws-billing-pp-cli search "error timeout"

  # Force local search only
  aws-billing-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  aws-billing-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  aws-billing-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'aws-billing-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/aws-billing-pp-cli/data.db)")
