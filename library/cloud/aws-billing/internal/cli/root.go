...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "aws-billing-pp-cli",
		Short: `Understand and trim your AWS bill from the terminal — per-account breakdowns, month-over-month deltas, and dollar-ranked waste, posted to Slack.`,
		Long: `Understand and trim your AWS bill from the terminal — per-account breakdowns, month-over-month deltas, and dollar-ranked waste, posted to Slack.
...
Highlights (not in the official API docs):
  ...
  • ask   Answers known questions about your bill (top services, biggest month-over-month mover, per-account total, transfer spend) straight from the local cache; emits the relevant slice as JSON for anything …

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'aws-billing-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("aws-billing-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.snark, "snark", false, "Add an opt-in Corey-Quinn-flavored voice to human output (suppressed under --json/--agent)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'aws-billing-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelAskCmd(flags))
	rootCmd.AddCommand(newNovelConsolidatedCmd(flags))
	rootCmd.AddCommand(newNovelIamSetupCmd(flags))
	rootCmd.AddCommand(newNovelWasteCmd(flags))
	rootCmd.AddCommand(newBillCmd(flags))
	rootCmd.AddCommand(newCompareCmd(flags))
	rootCmd.AddCommand(newForecastCmd(flags))
	rootCmd.AddCommand(newDimensionsCmd(flags))
