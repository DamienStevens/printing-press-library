...
func loadContactsPropertySchema(db *store.Store) (map[string]propertyDef, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("loading contacts property schema (run `hubspot-pp-cli sync` first?): %w", err)
	}
	defer rows.Close()
	for rows.Next() {
	...
	if len(out) == 0 {
		return nil, fmt.Errorf("no contacts property schema in the local store; run `hubspot-pp-cli sync` first")
	}
	return out, nil
}
